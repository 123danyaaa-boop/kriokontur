...
DEFAULT_ASSUMPTIONS = {
    ...
    # TEAM_ASSUMPTION: размер одной партии аварийного канала, т. None означает политику
    # по умолчанию «месячная доля годового договора × число шагов поставки». Это условие
    # договора, а не данные кейса: от него зависит, может ли контрактный резерв заменить
    # физический запас, поэтому параметр вынесен наружу и проверяется диапазоном.
    "emergency_batch_t": None,
    # TEAM_ASSUMPTION: режим диспетчеризации. reactive — месячный отбор пересматривается каждый
    # месяц по текущему состоянию; frozen — годовой график замораживается в начале года, внутри
    # года реагирует только аварийный канал со сроком поставки шесть недель. Второй режим честнее
    # отражает срок размещения заказа, первый показывает верхнюю границу качества управления.
