"""Автоматический подбор плана и сравнение с ручным планом команды.
    ...
    PYTHONPATH=src python scripts/optimize_plan.py
Пишет results/protocols/11_optimizer.md и сохраняет планы в configs/plans/optimized-*.json.
"""
import json
import statistics
import sys
...
def main() -> None:
    ...
    lines = ["# Протокол: автоматический подбор плана", "",
             ...
             "| План | PV BASE, млн | PV стресс, млн | Среднее по сценариям, млн | Худший сценарий, млн | Сценариев с нарушениями | Время, с | Инвестиции |",
             "|---|---|---|---|---|---|---|---|"]
    out = {}
    for key, label, v, secs, plan in rows:
        ...
        lines.append(f"| {label} | {v['BASE']['pv_mln']:,.1f} | {v['MANDATORY_STRESS']['pv_mln']:,.1f} | {statistics.fmean(pv):,.1f} | "
                     f"{max(pv):,.1f} | {bad} из {len(ids)} | {secs:.1f} | {inv} |".replace(",", " "))
        out[key] = {"label": label, "base": v["BASE"]["pv_mln"], "stress": v["MANDATORY_STRESS"]["pv_mln"],
                    "mean": statistics.fmean(pv), "worst": max(pv), "bad": bad, "time_s": round(secs, 2),
                    "problems": {k: x["codes"] for k, x in v.items() if x["hard"]}}
        print(f"{label:<50} BASE {v['BASE']['pv_mln']:9.1f}  стресс {v['MANDATORY_STRESS']['pv_mln']:9.1f}  нарушений {bad}/{len(ids)}")
    ...
    (ROOT / "results" / "protocols" / "11_optimizer.json").write_text(json.dumps(out, ensure_ascii=False, indent=1), encoding="utf-8")
